...
export async function markLeaderbotAiAnswerDeliveryStarted(input: {
  pageId: string;
  reservationId: string;
  ownerToken: string;
  deliveryAttemptToken: string;
}): Promise<boolean> {
  let response = await requestAiAnswerQuota("delivery-started", input);
  if (!response) response = await requestAiAnswerQuota("delivery-started", input);
  return response?.status === "delivery_started";
}

export async function markLeaderbotAiAnswerDeliveryKnownRejected(input: {
  pageId: string;
  reservationId: string;
  ownerToken: string;
  deliveryAttemptToken: string;
...
export async function heartbeatLeaderbotAiAnswerReservation(input: {
  reservationId: string;
  ownerToken: string;
}): Promise<boolean> {
