...
const fixedProbe = 'set -eu; printf "%s\\n" synthetic_restore_probe';
const manifestImage = JSON.parse(
  readFileSync(
    new URL("../deploy/production/apps.json", import.meta.url),
    "utf8",
  ),
).apps["image-gen"].databaseRecovery.mysqlImage;
const expected = {
  ...
  runAttempt: "2",
  image: manifestImage,
  probeSha256: createHash("sha256").update(fixedProbe).digest("hex"),
};
...
function machine() {
  return {
    ...
    config: {
      ...
      init: { entrypoint: ["/bin/sh"], cmd: ["-c", fixedProbe] },
      metadata: {
...
describe("isolated Fly restore probe status", () => {
  it("verifies one natural successful exit of the exact one-shot probe", () => {
    expect(expected.image).toBe(
      "docker-hub-mirror.fly.io/library/mysql:8.4.11@sha256:1d6b6a8fcee8ff758ff151d017f5203cd06792a0e698f0a593c9dfcb14609cf0",
    );
    expect(classifyFlyRestoreProbe(machine(), expected)).toBe("success");
  });

  it("accepts the reviewed mirror without its tag, preserving its digest", () => {
    const value = machine();
    value.config.image = expected.image.replace(":8.4.11@", "@");
    expect(classifyFlyRestoreProbe(value, expected)).toBe("success");
  });

  it.each(["docker.io", "registry-1.docker.io", "unreviewed-mirror.example"])(
    "does not equate the reviewed mirror with %s at the same digest",
    (registry) => {
      const value = machine();
      value.config.image = expected.image.replace(
        "docker-hub-mirror.fly.io",
        registry,
      );
      expect(classifyFlyRestoreProbe(value, expected)).toBe("failure");
      const mirrorValue = machine();
      expect(
        classifyFlyRestoreProbe(mirrorValue, {
          ...expected,
  ...
  it.each([
    `library/mysql:8.4.11@sha256:${"a".repeat(64)}`,
    `docker.io/library/mysql@sha256:${"a".repeat(64)}`,
    `registry-1.docker.io/library/mysql:8.4.11@sha256:${"a".repeat(64)}`,
  ])(
    "accepts only official MySQL aliases at the exact reviewed digest",
    (image) => {
      const value = machine();
      value.config.image = image;
      expect(
        classifyFlyRestoreProbe(value, {
          ...expected,
          image: `mysql:8.4.11@sha256:${"a".repeat(64)}`,
        }),
      ).toBe("success");
    },
  );
  ...
  it.each([
    (value) => {
      value.config.image = `mysql:8.4.11@sha256:${"b".repeat(64)}`;
    },
    (value) => {
      value.config.image = "mysql:8.4.11";
    },
    (value) => {
      value.config.image = `evil.example/library/mysql@sha256:${"a".repeat(64)}`;
    },
    (value) => {
      value.config.image = `docker.io/attacker/mysql@sha256:${"a".repeat(64)}`;
    },
    (value) => {
      value.config.image = null;
    },
    (value) => {
      value.config.init = null;
    },
    (value) => {
      value.config.init.entrypoint = ["/bin/true"];
    },
    (value) => {
      value.config.init.entrypoint = ["/bin/sh", "-c", "true"];
    },
    (value) => {
      value.config.init.entrypoint = "/bin/sh";
    },
    (value) => {
      value.config.init.cmd = ["-c", "true"];
    },
    (value) => {
      value.config.init.cmd = ["-c", `true; exit 0; ${fixedProbe}`];
    },
    (value) => {
      value.config.init.cmd = ["-c", `${fixedProbe}\n`];
    },
    (value) => {
      value.config.init.cmd = ["-lc", fixedProbe];
    },
    (value) => {
      value.config.init.cmd = ["-c", fixedProbe, "extra"];
    },
    (value) => {
      value.config.init.cmd = "true";
    },
    (value) => {
      value.config.init.cmd = ["-c", null];
    },
    (value) => {
      value.config.init.exec = ["/bin/true"];
    },
    (value) => {
      value.config.init.exec = "";
    },
    (value) => {
      value.config.init.kernel_args = ["init=/bin/true"];
    },
    (value) => {
      value.config.processes = [{ cmd: ["/bin/true"] }];
    },
    (value) => {
      value.config.containers = [
        { image: expected.image, exec: ["/bin/true"] },
      ];
    },
    (value) => {
      value.config.files = [
        { guest_path: "/bin/sh", raw_value: "replacement" },
      ];
    },
  ])(
    "rejects image or executable drift even with exact identity and exit zero",
    (mutate) => {
      const value = machine();
      mutate(value);
      expect(classifyFlyRestoreProbe(value, expected)).toBe("failure");
  ...
  it("accepts inert absent, null or empty alternate-execution fields", () => {
    const value = machine();
    value.config.init.exec = null;
    value.config.init.kernel_args = [];
    value.config.processes = [];
    value.config.containers = null;
    value.config.files = [];
    expect(classifyFlyRestoreProbe(value, expected)).toBe("success");
  });
  ...
  it.each([
    ...
    { ...expected, image: "mysql:8.4.11" },
    { ...expected, image: `attacker/mysql@sha256:${"a".repeat(64)}` },
    { ...expected, probeSha256: undefined },
    { ...expected, probeSha256: "A".repeat(64) },
    { ...expected, probeSha256: "a".repeat(63) },
  ])("rejects malformed expected bindings", (binding) => {
...
describe("restore probe CLI", () => {
  ...
  it.each([
    ...
    [JSON.stringify(machine()), args.slice(0, 8)],
    [
      JSON.stringify(machine()),
      [...args.slice(0, 10), "--expected-image", expected.image],
    ],
  ])(
  ...
  it("rejects a successful true-command without emitting the returned command", () => {
    const value = machine();
    value.config.init.cmd = ["-c", "true; private_configuration"];
    const result = run(value);
    expect(result.status).toBe(1);
    expect(result.stdout).toBe("mysql_restore_probe_failed\n");
    expect(result.stderr).toBe("");
...
describe("bounded read-only Machines API poll", () => {
  ...
  it.each([
    ...
    { ...binding, image: "mysql:latest" },
    { ...binding, probeSha256: undefined },
  ])(
