...
for (const abs of files) {
  ...
  if (file.startsWith("src/")) {
    for (const item of imports) {
      ...
      if (
        resolved.startsWith("apps/image-gen") ||
        item.specifier.includes("apps/image-gen")
      ) {
        report(
          "failure",
          "root src/ must not import from apps/image-gen",
          file,
          lineOf(source, item.index),
        );
      }
    }
    ...
    if (!rootSrcAllowLeaderbot.has(file) && !isTestFile(file)) {
      ...
      if (match)
        report(
          "failure",
          "root src/ may mention leaderbot/LEADERBOT_ only in src/leaderbot-bridge.ts or tests",
          file,
          lineOf(source, match.index),
        );
    } else if (
      file !== "src/leaderbot-bridge.ts" &&
      !isTestFile(file) &&
      /leaderbot|LEADERBOT_/i.test(source)
    ) {
      report(
        "warning",
        "legacy allowlist: leaderbot string outside src/leaderbot-bridge.ts; migrate toward the bridge boundary",
        file,
      );
    }
  }
  ...
  if (file.startsWith("apps/image-gen/server/_core/")) {
    for (const item of imports) {
      const resolved = resolveImport(file, item.specifier);
      const importsBlockedChannelModule =
        /(^|\/)(messengerApi|whatsappApi|messengerActionRenderer)(\.[cm]?[jt]sx?)?$/.test(
          resolved,
        );
      if (importsBlockedChannelModule) {
        if (coreChannelImportAllowlist.has(file)) {
          report(
            "warning",
            "legacy allowlist: _core shared/conversation code imports channel-specific module",
            file,
            lineOf(source, item.index),
          );
        } else {
          report(
            "failure",
            "_core conversation/shared modules must not import messengerApi, whatsappApi, or messengerActionRenderer",
            file,
            lineOf(source, item.index),
          );
        }
      ...
      const importsMessengerQuota =
        /(^|\/)messengerQuota(\.[cm]?[jt]sx?)?$/.test(resolved);
      const isWhatsappModule =
        file.includes("/whatsapp") ||
        path.posix.basename(file).startsWith("whatsapp");
      if (isWhatsappModule && importsMessengerQuota) {
        if (whatsappMessengerQuotaAllowlist.has(file))
          report(
            "warning",
            "legacy allowlist: WhatsApp module imports messengerQuota directly",
            file,
            lineOf(source, item.index),
          );
        else
          report(
            "failure",
            "WhatsApp modules must not import messengerQuota directly; use a subject/wrapper quota boundary",
            file,
            lineOf(source, item.index),
          );
      }
    ...
    while ((match = rawPsidPattern.exec(source))) {
      if (rawPsidKeyAllowlist.has(file))
        report(
          "warning",
          "legacy allowlist: raw psid: storage key helper; new state/quota modules should use subject/wrapper helpers",
          file,
          lineOf(source, match.index),
        );
      else
        report(
          "failure",
          "new state/quota modules must not add raw psid: key helpers; use subject/wrapper helpers",
          file,
          lineOf(source, match.index),
        );
    }
...
console.log(
  `Product boundary check passed with ${warnings.length} warning(s).`,
);
