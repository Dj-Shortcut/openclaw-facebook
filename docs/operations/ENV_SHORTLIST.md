...
| `MESSENGER_SHARED_STATE_REDIS_URL` | Root OpenClaw Facebook gateway shared dedupe and daily caps | Required only when `channels.facebook.sharedStateStore` is `redis`; use `redis://` or `rediss://`. Selection is root-only and startup fails when Redis is unavailable. |
...
| `MESSENGER_ADMIN_IDS` | Exact comma-separated owner PSIDs | Owner-only bypass for customer image quota, entitlement, and customer daily/monthly spend caps. Owner attempts remain in aggregate cost reporting but are excluded from customer-cap calculations. Keep the list minimal and never log its raw values. |
| `MESSENGER_OWNER_EMERGENCY_DAILY_SPEND_CAP_USD`, `MESSENGER_OWNER_EMERGENCY_MONTHLY_SPEND_CAP_USD` | Independent global hard owner cost stops | Bound the combined usage of all configured owner identities without applying customer limits. Defaults are `$10` daily and `$100` monthly; production declares both explicitly. Owner provider attempts fail closed when unpriced. |
