...
| `MESSENGER_ADMIN_IDS` | Exact comma-separated owner PSIDs | Owner-only bypass for customer image quota, entitlement, and daily/monthly spend caps. Owner attempts remain in aggregate cost reporting but are excluded from customer-cap calculations. Keep the list minimal and never log its raw values. |
