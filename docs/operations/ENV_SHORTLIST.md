...
| `MESSENGER_OWNER_EMERGENCY_DAILY_SPEND_CAP_USD`, `MESSENGER_OWNER_EMERGENCY_MONTHLY_SPEND_CAP_USD` | Independent hard owner cost stops | Bound compromised owner credentials without applying customer limits. Defaults are `$100` daily and `$1000` monthly; production declares both explicitly. Owner provider attempts fail closed when unpriced. |
