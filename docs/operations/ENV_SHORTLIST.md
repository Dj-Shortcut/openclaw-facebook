...
| `LEADERBOT_AI_ANSWER_ENFORCEMENT_ENABLED` | Gateway preflight for the finite 300-answer Startpilot quota | Defaults off, so ordinary free OpenClaw chat has no image-gen dependency. When enabled, gateway startup requires the authenticated `leaderbot-ai-answer-quota-v1` readiness handshake with preflight, admission and drain all ready. Runtime lease, delivery-fence or quota failures fail closed before Graph transport. Enable only after migration and verified image-gen entitlement/finalization readiness. |
