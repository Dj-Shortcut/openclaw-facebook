...
| `LEADERBOT_AI_ANSWER_ENFORCEMENT_ENABLED` | Optional future OpenClaw answer quota | Keep false for the image-generation Startpilot launch: AI answers are not part of the public offer and the Mollie sandbox/payment route must not depend on the gateway. A later reviewed OpenClaw rollout may enable the authenticated `leaderbot-ai-answer-quota-v1` contract after its own state, migration, rollback and readiness proof. |
