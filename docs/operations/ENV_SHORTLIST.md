...
| Variable                                              | Required for                                                                       | Notes                                                                                                                                                                                                      |
| ----------------------------------------------------- | ---------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `OPENAI_API_KEY`                                      | Image generation, conversational edit interpretation, and enabled video generation | If missing, image/video generation fails closed and edit interpretation is skipped.                                                                                                                        |
...
| `MESSENGER_VIDEO_GENERATION_ENABLED`                  | Owner-operated photo-to-video beta                                                 | Keep `false` until Messenger review and the complete bounded policy below are approved. Enabling it makes every listed video control mandatory at startup.                                                 |
| `MESSENGER_VIDEO_ALLOWED_USER_KEYS`                   | Bounded video-pilot audience                                                       | Required when enabled in production. Use unique comma-separated 64-character pseudonymous user keys; never raw PSIDs.                                                                                      |
| `MESSENGER_VIDEO_ALLOWED_PAGE_BINDINGS`               | Exact owner Page pilot boundary                                                    | Required when enabled. Use unique `workspace:connection:binding:Page` tuples so the runtime-wide flag cannot admit another connected Page.                                                                 |
| `MESSENGER_VIDEO_PROVIDER`                            | Server-owned video provider                                                        | Initial reviewed rollout requires exact value `openai`; Messenger and browser input cannot override it.                                                                                                    |
| `OPENAI_VIDEO_MODEL`                                  | Server-owned video model                                                           | Initial reviewed rollout requires exact value `sora-2`; a change needs new unit economics and rollback review.                                                                                             |
| `OPENAI_VIDEO_SIZE`                                   | Server-owned video dimensions                                                      | Initial reviewed rollout requires exact value `1280x720`.                                                                                                                                                  |
| `OPENAI_VIDEO_SECONDS`                                | Server-owned video duration                                                        | Initial reviewed rollout requires exact value `8`.                                                                                                                                                         |
| `OPENAI_VIDEO_MAX_RETRIES`                            | Video-provider retry count                                                         | Initial reviewed rollout requires exact value `0`; one user request can start at most one billable video job.                                                                                              |
| `OPENAI_VIDEO_MAX_OUTPUT_BYTES`                       | Downloaded MP4 memory cap                                                          | Initial reviewed rollout requires exact value `25165824` (24 MiB); streaming stops as soon as this cap is exceeded.                                                                                        |
| `OPENAI_VIDEO_MAX_REFERENCE_IMAGE_BYTES`              | Provider reference-image memory cap                                                | Initial reviewed rollout requires exact value `12582912` (12 MiB).                                                                                                                                         |
| `MESSENGER_TTS_ENABLED`                               | Legacy video voice-over path                                                       | Must be exact `false` for the initial rollout; speech synthesis and muxing have separate cost and failure boundaries that are not part of this feature.                                                    |
| `MESSENGER_VIDEO_AMBIGUOUS_CREATE_RETENTION_APPROVED` | Owner approval for provider create-timeout residual                                | Must be exact `true` before enabling. The provider has no documented idempotency/recovery key for video create; a timeout before job-id receipt can remain provider-side for its documented 30-day window. |
| `MESSENGER_VIDEO_GENERATION_DAILY_LIMIT`              | Per-user video provider-attempt quota                                              | Initial reviewed rollout requires exact value `1`; admin and quota-bypass lists do not bypass this video pilot cap.                                                                                        |
| `MESSENGER_GLOBAL_DAILY_VIDEO_CAP`                    | Global Messenger video provider-attempt cap                                        | Must be a positive integer when enabled and is reserved before provider work.                                                                                                                              |
| `OPENAI_VIDEO_GENERATION_ESTIMATED_COST_USD`          | Estimated cost of one video provider attempt                                       | Must be a positive reviewed amount when enabled so global/monthly/per-user spend caps fail closed before provider work.                                                                                    |
| `MESSENGER_VIDEO_GENERATION_TIMEOUT_MS`               | Per-provider video deadline                                                        | Must be a positive integer when enabled; initial value is `240000`.                                                                                                                                        |
