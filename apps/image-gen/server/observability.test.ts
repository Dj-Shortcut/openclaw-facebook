  ...
  it("exposes Prometheus-style HTTP metrics", async () => {
    ...
    try {
      ...
      expect(body).toContain(
        'webhook_ack_duration_seconds_bucket{channel="facebook",mode="queued",le="0.25"} 1'
      );
      expect(body).toContain(
        'webhook_ack_duration_seconds_sum{channel="facebook",mode="queued"} 0.123000'
      );
      expect(body).toContain(
        'webhook_ack_duration_seconds_count{channel="facebook",mode="queued"} 1'
      );
      ...
      expect(body).toContain(
        'messenger_generation_queue_jobs{state="queued"} 0'
      );
      expect(body).toContain(
        'messenger_generation_queue_jobs{state="processing"} 0'
      );
      expect(body).toContain(
        'messenger_generation_queue_jobs{state="failed"} 0'
      );
      expect(body).toContain(
        'messenger_generation_global_slots{state="active"} 0'
      );
      expect(body).toContain(
        'messenger_generation_global_slots{state="max"} 3'
      );
      expect(body).toContain(
        "messenger_generation_global_slots_redis_backed 0"
      );
      expect(body).toContain(
        "messenger_generation_daily_spend_budget_enabled 0"
      );
      expect(body).toContain(
        "messenger_generation_daily_spend_budget_cap_usd 0"
      );
      expect(body).toContain(
        "messenger_generation_monthly_spend_budget_enabled 0"
      );
      expect(body).toContain(
        "messenger_generation_monthly_spend_budget_cap_usd 0"
      );
      expect(body).toContain(
        "messenger_generation_user_daily_spend_budget_enabled 0"
      );
      expect(body).toContain(
        "messenger_generation_user_daily_spend_budget_cap_usd 0"
      );
      expect(body).toContain("messenger_generation_queue_enabled 0");
      expect(body).toContain(
        'messenger_generation_queue_jobs{state="queued"} 0'
      ...
      expect(body).toContain(
        "messenger_generation_average_latency_seconds 1.234000"
      );
    } finally {
      await server.close();
    }
