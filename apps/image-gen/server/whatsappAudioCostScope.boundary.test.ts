...
import { readCostLedgerPeriod, type CostLedgerScope } from "./_core/costLedger";
...
const SCOPE: CostLedgerScope = Object.freeze({
  workspaceId: 42,
...
describe("WhatsApp audio cost-admission boundary", () => {
  ...
  it("records the exact WhatsApp tenant scope before the provider call", async () => {
    ...
    const entries = await readCostLedgerPeriod(SCOPE, period);
