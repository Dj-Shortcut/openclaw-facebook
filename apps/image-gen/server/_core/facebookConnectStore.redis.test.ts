...
describe("Facebook connect state token storage", () => {
  ...
  it("defaults to legacy writes during the mixed old/new bridge deploy", async () => {
    vi.stubEnv("FACEBOOK_CONNECT_STORAGE_MODE", "");
    const codeState = await startFacebookConnect({
      workspaceId: 42,
      userId: 7,
      now: Date.now(),
    });
    ...
    const codeSerialized =
      redis.values.get(`portal:facebook_connect:${codeState.state}`) ?? "";
    expect(JSON.parse(codeSerialized)).toMatchObject({
      authorizationCode: RAW_AUTHORIZATION_CODE,
    });
    expect(codeSerialized).not.toContain("fca1:");
    await expect(
      validateStoredFacebookState({
        state: codeState.state,
        workspaceId: 42,
    ...
    ).resolves.toMatchObject({ authorizationCode: RAW_AUTHORIZATION_CODE });

    const pageState = await storePage();
    const pageSerialized =
      redis.values.get(`portal:facebook_connect:${pageState.state}`) ?? "";
    expect(JSON.parse(pageSerialized).pages[0].accessToken).toBe(
      RAW_PAGE_TOKEN
    );
    expect(pageSerialized).not.toContain("fc1:");
    await expect(
      consumeFacebookPage({
    ...
    ).resolves.toMatchObject({ accessToken: RAW_PAGE_TOKEN });
  });

  it("rejects an invalid production mode before writing connect state", async () => {
    vi.stubEnv("NODE_ENV", "production");
    vi.stubEnv("FACEBOOK_CONNECT_STORAGE_MODE", "sealed_immediately");

    await expect(
      startFacebookConnect({ workspaceId: 42, userId: 7 })
    ).rejects.toThrow(
  ...
  it("dual-reads records written by the old plaintext implementation", async () => {
    vi.stubEnv("FACEBOOK_CONNECT_STORAGE_MODE", "sealed_compat");
    const codeState = "old-writer-authorization-state";
    redis.values.set(
      `portal:facebook_connect:${codeState}`,
      JSON.stringify({
        state: codeState,
        workspaceId: 42,
        userId: 7,
    ...
    const pageState = "old-writer-page-state";
    redis.values.set(
      `portal:facebook_connect:${pageState}`,
      JSON.stringify({
        state: pageState,
        workspaceId: 42,
        userId: 7,
  ...
  it("writes sealed records in sealed_compat and dual-reads them after rollback", async () => {
    vi.stubEnv("FACEBOOK_CONNECT_STORAGE_MODE", "sealed_compat");
    const state = await startFacebookConnect({
      workspaceId: 42,
      userId: 7,
    ...
    const serialized =
      redis.values.get(`portal:facebook_connect:${state.state}`) ?? "";
    expect(serialized).not.toContain(RAW_AUTHORIZATION_CODE);
    expect(JSON.parse(serialized).authorizationCodeEnvelope).toMatch(/^fca1:/);

    vi.stubEnv("FACEBOOK_CONNECT_STORAGE_MODE", "legacy_compat");
    await expect(
      validateStoredFacebookState({
        state: state.state,
        workspaceId: 42,
  ...
  it("fails closed when the Page-token envelope is tampered with", async () => {
    const state = await storePage();
    const key = `portal:facebook_connect:${state.state}`;
    const stored = JSON.parse(redis.values.get(key) ?? "{}");
  ...
  it("binds each Page-token envelope to state, tenant, user, and Page", async () => {
    const state = await storePage();
    const originalKey = `portal:facebook_connect:${state.state}`;
    const original = JSON.parse(redis.values.get(originalKey) ?? "{}");
    const cases = [
      {
        state: "different-facebook-connect-state",
        workspaceId: 42,
        userId: 7,
        pageId: "page-42",
      },
    ...
    for (const context of cases) {
      ...
      moved.state = context.state;
      moved.workspaceId = context.workspaceId;
      moved.userId = context.userId;
      moved.pages[0].id = context.pageId;
      redis.values.clear();
      redis.values.set(
        `portal:facebook_connect:${context.state}`,
        JSON.stringify(moved)
      );

      await expect(consumeFacebookPage(context)).rejects.toThrow(
  ...
  it("rejects storage when key material is absent or too short", async () => {
    const state = await startFacebookConnect({
      workspaceId: 42,
      userId: 7,
      now: Date.now(),
    ...
    ).rejects.toThrow("JWT_SECRET must be at least 32 bytes");
    expect(
      redis.values.get(`portal:facebook_connect:${state.state}`)
    ).not.toContain(RAW_AUTHORIZATION_CODE);
  });
  ...
  it("deletes a legacy plaintext authorization code without refreshing its TTL", async () => {
    ...
    const logs = [logSpy, warnSpy, errorSpy]
      .flatMap(spy => spy.mock.calls)
      .flat()
      .join(" ");
    expect(logs).not.toContain(RAW_AUTHORIZATION_CODE);
  });
