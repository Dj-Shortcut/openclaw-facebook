...
const originalStorageMode = process.env.FACEBOOK_CONNECT_STORAGE_MODE;
...
suite("Facebook connect token sealing with Redis", () => {
  beforeAll(async () => {
    ...
    process.env.FACEBOOK_CONNECT_STORAGE_MODE = "sealed_only";
  ...
  afterAll(() => {
    ...
    if (originalStorageMode === undefined) {
      ...
      process.env.FACEBOOK_CONNECT_STORAGE_MODE = originalStorageMode;
    }
  });
  ...
  it("rejects envelopes under different key material", async () => {
    const { connectState, key } = await storePageInRedis();
