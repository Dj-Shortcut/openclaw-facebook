DROP PROCEDURE IF EXISTS `credit_reserve_checkout_intent`;--> statement-breakpoint
...
credit_reserve_checkout_intent_body: BEGIN
	...
	IF NOT (
		...
		AND (
			(BINARY p_offer_snapshot_code=BINARY 'premium_images_8_medium_v1'
				AND p_expected_amount=4.99 AND p_credit_count=8
				AND BINARY p_description=BINARY 'Leaderbot - 8 premium beeldcredits')
			OR
			(BINARY p_offer_snapshot_code=BINARY 'premium_images_9_medium_v2'
				AND p_expected_amount=5.00 AND p_credit_count=9
				AND BINARY p_description=BINARY 'Leaderbot - 9 premium beeldcredits')
		)
	...
	IF v_existing_wallet_id IS NOT NULL THEN
		...
		WHERE BINARY `wallet_id`=BINARY p_wallet_id AND `workspace_id`=p_workspace_id AND `mode`=p_mode
			...
			AND `status`='active' AND `refund_adjustment_entry_id` IS NULL
			AND `privacy_erased_at` IS NULL FOR UPDATE;
	...
	IF BINARY p_offer_snapshot_code=BINARY 'premium_images_8_medium_v1'
		AND v_existing_intent_id IS NULL THEN
		SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT='legacy credit offer is replay-only';
	END IF;
...
DROP PROCEDURE IF EXISTS `credit_freeze_wallet_for_review`;--> statement-breakpoint
...
credit_freeze_body: BEGIN
	...
	WHERE BINARY intent.`intent_id`=BINARY p_intent_id
		...
		AND ((BINARY intent.`plan_code`=BINARY 'premium_images_8_medium_v1'
			AND intent.`expected_amount`=4.99 AND intent.`credit_count`=8
			AND BINARY intent.`mollie_description`=BINARY 'Leaderbot - 8 premium beeldcredits')
			OR (BINARY intent.`plan_code`=BINARY 'premium_images_9_medium_v2'
			AND intent.`expected_amount`=5.00 AND intent.`credit_count`=9
			AND BINARY intent.`mollie_description`=BINARY 'Leaderbot - 9 premium beeldcredits'))
		AND BINARY intent.`currency`=BINARY 'EUR' AND intent.`interval`='oneoff'
		...
		AND payment.`gross_amount`=intent.`expected_amount`
		AND BINARY payment.`currency`=BINARY intent.`currency`
